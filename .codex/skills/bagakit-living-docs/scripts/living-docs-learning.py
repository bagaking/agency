...
Goal: turn agent session JSONL logs into draft inbox memory entries under
...
def extract_text(content: Any) -> str:
    """
    Session JSONL stores message content as a list of blocks like:
...
def guess_agent_home() -> Path:
    env = os.environ.get("BAGAKIT_AGENT_HOME")
    if env:
        return Path(env).expanduser()
    env = os.environ.get("BAGAKIT_HOME")
    if env:
        return Path(env).expanduser()
    return Path("~/.bagakit").expanduser()


def find_sessions(agent_home: Path) -> List[Path]:
    base = agent_home / "sessions"
    if not base.is_dir():
        return []
    # Sessions are stored in subdirs by date.
...
def cmd_sessions(args: argparse.Namespace) -> int:
    ...
    files = find_sessions(agent_home)
    if not files:
        eprint(f"no sessions found under: {agent_home / 'sessions'}")
        return 2
...
def cmd_extract(args: argparse.Namespace) -> int:
    ...
    if args.session:
        ...
        agent_home = Path(args.agent_home).expanduser()
        sessions = find_sessions(agent_home)
        if not sessions:
            eprint(f"no sessions found under: {agent_home / 'sessions'}")
            return 2
    ...
    if args.topic:
        ...
        topic = f"learning-{day.replace('-', '')}"

    title = args.title or f"Daily learnings ({day})"
...
def main(argv: List[str]) -> int:
    p = argparse.ArgumentParser(prog="living-docs-learning.py")
    sub = p.add_subparsers(dest="cmd", required=True)

    p_sessions = sub.add_parser("sessions", help="List recent agent session JSONL paths.")
    p_sessions.add_argument("--agent-home", default=str(guess_agent_home()), help="Agent home dir (default: $BAGAKIT_AGENT_HOME or ~/.bagakit)")
    ...
    p_extract.add_argument("--last", action="store_true", help="Use the most recent session under --agent-home.")
    p_extract.add_argument("--agent-home", default=str(guess_agent_home()), help="Agent home dir (default: $BAGAKIT_AGENT_HOME or ~/.bagakit)")
