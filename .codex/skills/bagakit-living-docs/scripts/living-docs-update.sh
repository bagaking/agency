...
usage() {
  cat >&2 <<'EOF'
Usage:
  sh scripts/living-docs-update.sh status [--root <project_root>] [--skill-dir <dir>] [--repo <git_url>] [--branch <name>]
  sh scripts/living-docs-update.sh apply  [--root <project_root>] [--skill-dir <dir>] [--overwrite] [--vendor-scripts]
  sh scripts/living-docs-update.sh update-skill [--skill-dir <dir>] [--branch <name>]
...
1) --skill-dir <dir>
2) $BAGAKIT_LIVING_DOCS_SKILL_DIR
3) $BAGAKIT_HOME/skills/bagakit-living-docs (if BAGAKIT_HOME is set)
4) ~/.bagakit/skills/bagakit-living-docs (if exists)
5) fallback: ~/.bagakit/skills/bagakit-living-docs
EOF
  exit 1
}
...
resolve_skill_dir() {
  ...
  if [ -n "${BAGAKIT_LIVING_DOCS_SKILL_DIR:-}" ]; then
    printf '%s\n' "$BAGAKIT_LIVING_DOCS_SKILL_DIR"
    return 0
  fi
  if [ -n "${BAGAKIT_HOME:-}" ]; then
    printf '%s\n' "${BAGAKIT_HOME}/skills/bagakit-living-docs"
    return 0
  fi
  if [ -d "$HOME/.bagakit/skills/bagakit-living-docs" ]; then
    printf '%s\n' "$HOME/.bagakit/skills/bagakit-living-docs"
    return 0
  fi
  printf '%s\n' "$HOME/.bagakit/skills/bagakit-living-docs"
}
...
# If this updater is invoked from a project that already has vendored scripts,
# default to keeping them refreshed unless the user explicitly opts out.
if [ "$vendor_scripts_set" -eq 0 ]; then
  if [ -f "$root_abs/scripts/living-docs-update.sh" ] && [ -f "$root_abs/scripts/validate-docs.sh" ]; then
    vendor_scripts=1
  fi
fi
...
case "$cmd" in
  status)
    ...
    if [ -n "${lh:-}" ]; then
      if [ "$lh" = "$rh" ]; then
        ...
        echo "  sh scripts/living-docs-update.sh update-skill --skill-dir \"$sd\" --branch \"$branch\""
        echo "hint: then re-apply into this project:"
        echo "  sh scripts/living-docs-update.sh apply --root \"$root_abs\""
      fi
    else
      echo "status: remote queried; local version unknown (not a git checkout)"
      echo "hint: for a comparable local version, install the skill as a git checkout and set BAGAKIT_LIVING_DOCS_SKILL_DIR to it."
    fi
    ;;
  ...
  apply)
    if [ ! -f "$apply" ]; then
      ...
      echo "hint: install the skill into \$BAGAKIT_HOME/skills/bagakit-living-docs, or pass --skill-dir /path/to/bagakit-living-docs" >&2
      exit 2
    fi
    vendor_arg=""
    if [ "$vendor_scripts" -eq 1 ]; then
      vendor_arg="--vendor-scripts"
    fi
    if [ "$overwrite" -eq 1 ]; then
      exec bash "$apply" "$root_abs" --force $vendor_arg
    fi
    exec bash "$apply" "$root_abs" $vendor_arg
    ;;
