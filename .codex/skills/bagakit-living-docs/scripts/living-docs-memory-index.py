...
def cmd_index(args: argparse.Namespace) -> int:
    ...
    try:
        try:
            ...
            print(
                "hint: use `export BAGAKIT_LIVING_DOCS_SKILL_DIR=\"${BAGAKIT_LIVING_DOCS_SKILL_DIR:-${BAGAKIT_HOME:-$HOME/.bagakit}/skills/bagakit-living-docs}\" && "
                "sh \"$BAGAKIT_LIVING_DOCS_SKILL_DIR/scripts/living-docs-memory.sh\" search ... --root .` (scan mode), "
                "or install a Python/SQLite build with FTS5."
            )
            return 2
...
def cmd_search(args: argparse.Namespace) -> int:
    ...
    if not db_path.is_file():
        ...
        print(
            "hint: run `export BAGAKIT_LIVING_DOCS_SKILL_DIR=\"${BAGAKIT_LIVING_DOCS_SKILL_DIR:-${BAGAKIT_HOME:-$HOME/.bagakit}/skills/bagakit-living-docs}\" && "
            "python3 \"$BAGAKIT_LIVING_DOCS_SKILL_DIR/scripts/living-docs-memory-index.py\" index --root .` first."
        )
        return 2
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="living-docs-memory-index.py")
