...
upsert_updated_field() {
  # Ensure frontmatter has an "updated:" field and set it to a UTC timestamp.
  ...
  awk -v ts="$ts" '
    ...
    in_fm==1 && $0=="---" {
      if (!has_updated) print "updated: " ts
      in_fm=0
      done=1
      print
      next
    ...
    in_fm==1 {
      if ($0 ~ /^updated:[[:space:]]+/) { print "updated: " ts; has_updated=1; next }
      print
      next
    }
    { print }
...
EOF
  ...
  promote)
    ...
    [ -n "$topic" ] || topic="promoted-$(today_date)"
