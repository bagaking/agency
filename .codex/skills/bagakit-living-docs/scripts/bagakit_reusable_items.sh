...
usage() {
  cat >&2 <<'EOF'
Usage:
  sh scripts/bagakit_reusable_items.sh list [--root <project_root>] [--domain <d>] [--level MUST|SHOULD|NICE] [--format text|json]
  sh scripts/bagakit_reusable_items.sh search <query> [--root <project_root>] [--domain <d>] [--level MUST|SHOULD|NICE] [--max-results N] [--format text|json]
...
py="$(dirname "$0")/bagakit_reusable_items.py"
