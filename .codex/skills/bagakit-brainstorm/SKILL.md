...
- Default mode (no driver system detected): generate one unified local artifact under `.bagakit/brainstorm/outcome/brainstorm-handoff-<slug>.md` (shared for action+memory handoff).
- Optional adapter routes: feat-harness, openspec, living-docs inbox/memory.
- Route selection is rule-driven and fallback-safe; never hard-coupled.
...
- If an experiment has multiple MVP versions, enforce version layout under the experiment directory:
  ...
  - do not use `versions/` nesting or `candidate-vN.md` files outside version folders
...
- Validation split:
  - script gates validate objective invariants (required files/sections/status/destination evidence),
  - qualitative quality (question depth, forum rigor, writing quality) is reviewed via prompt rubrics by coding agent/human.
...
- If no driver systems are detected, fallback to local unified artifact `.bagakit/brainstorm/outcome/brainstorm-handoff-<slug>.md` and archive the exact path.
...
```text
[[BAGAKIT]]
- LivingDoc: Evidence written to planning artifacts and matrix sections.
