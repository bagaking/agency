...
- Script validation should only gate objective invariants (required sections/status fields/destination evidence).
- Qualitative quality (question depth, debate rigor, writing quality) should be reviewed via prompt rubric by coding agent/human.
...
- Brainstorm completion produces two handoffs:
  ...
  - memory handoff (to living-docs inbox/memory or local summary fallback).
...
- Expert-forum review is a default hard gate before completion:
  ...
  - persona mix should cover deep thinker + creative explorer + constructive challenger.
  - deep-dive/lightning modes require expert web references and cross-scoring rows (`0~10`).
...
1. `feat-harness` (when detected and feat id resolved)
2. `openspec` (when detected and change id resolved)
3. local fallback unified handoff (`.bagakit/brainstorm/outcome/brainstorm-handoff-<slug>.md`) only when no driver is detected

### Memory handoff priority

1. `bagakit-living-docs` inbox (`docs/.bagakit/inbox/`) when detected
2. local fallback unified handoff (same file as action fallback under `.bagakit/brainstorm/outcome/`)
