...
- `Archived` is a first-class lifecycle surface: once a Cell enters lifecycle state `archived`, it must leave active/cleanup buckets and become reachable through an explicit `View Archived` affordance; detached archived Cells may present offline copy, but archive is still a lifecycle surface rather than a cleanup afterthought. `Archive Cell` is only the cleanup-triage CTA, while archived cards bias toward review/details and keep `Delete Cell` plus attachment-metadata cleanup in the selected Cell details pane.
