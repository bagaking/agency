...
- [ ] With a project open, use the title-bar project-path copy affordance and confirm the full project path is copied with inline confirmation rather than a popup while the rest of the summary rail still behaves like drag chrome.
...
- [ ] Force a browser-lane sync while comments drawer / attention rail are open, then confirm the browser still hides if the renderer-view seam cannot be resolved instead of preserving stale placement.
