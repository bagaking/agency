...
function resolveToneClasses(item: AttentionItem | null | undefined): string {
  switch (item?.kind) {
    case 'failed':
...
export function AttentionPill({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.14em] ${resolveToneClasses(item)} ${className}`}
    >
      <span>{resolveLabel(item)}</span>
      {normalizedCount > 1 ? (
        <span className="rounded-full bg-black/20 px-1 py-[1px] text-[8px] leading-none">
          {normalizedCount}
        </span>
      ) : null}
    </span>
