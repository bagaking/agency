  ...
  return (
    <div className={`relative inline-flex items-center ${className}`.trim()}>
      <button
        ...
        className={`group inline-flex min-h-8 items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.15em] transition-colors ${
          disabled
      ...
      >
        ...
        <span className="max-w-36 truncate text-[10px] font-black tracking-[0.13em] text-white/85">
          {selectedLabel}
        </span>
        <span
          className={`rounded-full border px-1.5 py-0.5 text-[8px] font-bold tracking-[0.14em] ${sourceTone}`}
      ...
      {open ? (
        <div
          ...
          className="absolute bottom-full right-0 z-30 mb-2 w-80 rounded-xl border border-white/15 bg-[#0f131b]/95 p-3 shadow-2xl backdrop-blur-xl"
        ...
        >
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              ...
              className={`inline-flex h-6 items-center gap-1 rounded-full border border-white/15 px-2 text-[9px] font-bold uppercase tracking-[0.12em] text-white/55 transition-colors hover:border-white/30 hover:text-white/90 ${focusRing.dark}`}
            >
              Close
            </button>
          </div>
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <input
              ...
              className={`h-8 w-full rounded-lg border border-white/15 bg-black/25 px-2.5 text-[11px] text-white/90 placeholder:text-white/35 ${focusRing.dark}`}
            />
            {canResetToAuto ? (
              <button
                type="button"
                onClick={handleResetToAuto}
                className={`inline-flex h-8 shrink-0 items-center gap-1 rounded-lg border border-amber-300/35 bg-amber-500/10 px-2.5 text-[9px] font-black uppercase tracking-[0.11em] text-amber-100 transition-colors hover:bg-amber-400/20 ${focusRing.dark}`}
          ...
          {errorText ? (
            <div
              className="mt-3 rounded-lg border border-rose-300/35 bg-rose-500/10 px-2.5 py-2 text-[10px] text-rose-100"
