...
- The workbench supports multi-tab previews, quick open, diff/blame toggles, media previews, active-tab disk-change auto sync (auto-reload when clean, warning + reload when dirty), and project-level language rules from `.agency/workbench.yaml` / `.agency/workbench.yml`.
