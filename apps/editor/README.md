...
- Explorer includes a bounded `URL` research mode that launches a Workbench-hosted web research tab for public URL inspection. That tab owns a true `View` browser surface plus `Reader`, keeps the browser content as the dominant panel with a compact toolbar for address/mode/actions, routes saved Markdown through fixed source frontmatter and memo citation flows, stays limited to public `http/https`, and keeps cookies/session/tab management out of scope while preserving the explicit system-browser escape hatch.
...
- Research lane stays subordinate to Explorer/file workflow: Explorer owns URL intake, Workbench owns the bounded browser/research object, the native `View` host stays limited to public `http/https` pages, reader previews and page-level actions stay inside the Workbench tab, citations reuse HIL memo artifacts, and full browsing still escapes to the system browser via the explicit `Open in Browser` action rather than growing general browser chrome inside Agency.
