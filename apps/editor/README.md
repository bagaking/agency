...
- Explorer includes a bounded `URL` research mode that launches a Workbench-hosted web research tab for public URL inspection. That tab owns a true `View` browser surface plus `Reader`, keeps the browser content as the dominant panel with a compact two-tier toolbar (`Back`/`Forward`/`Reload`/address/mode first, research actions second), anchors the native browser to one Workbench-owned viewport host with an explicit renderer-view seam instead of relaying geometry through tab-local overlays, routes saved Markdown through fixed source frontmatter and memo citation flows, stays limited to public `http/https`, and keeps cookies/session/tab management out of scope while preserving the explicit system-browser escape hatch.
...
- Research lane stays subordinate to Explorer/file workflow: Explorer owns URL intake, Workbench owns the bounded browser/research object, the native `View` host stays limited to public `http/https` pages, basic navigation controls stay inside the first browser toolbar row, research actions stay in a secondary row, citations reuse HIL memo artifacts, and full browsing still escapes to the system browser via the explicit `Open in Browser` action rather than growing general browser chrome inside Agency.
...
- Agent Cells uses one shared tactical rail-card grammar across tracked, detached, unmanaged, and legacy compatibility surfaces. The surface should not drift back to bright rounded pills, white outline boxes, white inset edge highlights, or per-section control families that read like stock dashboard cards.
