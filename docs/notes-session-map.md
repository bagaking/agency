...
- **HUD 分区（Dock 模式）**：左侧 Radar、中央 Command Center、右侧 Focus（头像/状态）；Dock 默认高度只容纳 3 行 token。Command Center 以 SLG 网格卡片分组展示 Cell（每个卡片内为在线 sessions），离线/不活跃会话折叠到 “…” 菜单。
...
- **展开/收起**：Dock 模式支持扩展到“高占比”，收起后固定为 3 行 token 高度（约 170px）。
