...
- Reply 记录为 session-owned artifact，canonical 地落在 `.agency/cells/<cellId>/session-replies/`；legacy worktree/HIL reply 仅作迁移来源。它不是 HIL item，也不进入 Memo inbox / Promote 选择树。
...
- **Tracked / Detached / Unmanaged grammar**：Agent Cells 默认用三类 core workspace 管理分组：
  - `Tracked Cells`：包括 live worktree attachments，以及仍运行在 project root 的 project-root Cells；
  - `Detached Cells`：Cell 记录仍在，但 attachment 为 `detached / missing`；
  - `Unmanaged Worktrees`：repo 中 live worktree 存在，但尚未绑定 Cell。
- **Project-root Cell surface**：project-root 不是错误态，也不是 detached cleanup。它代表 “Cell identity 已存在，并且当前 runtime root 就是 project root”。它既可以无 branch，也可以先绑定 branch metadata；主动作是 `Create Session`，增强动作才是 `Bind Branch` / `Create Worktree Attachment`。
- **Project-root runtime**：project-root Cell 仍应是 session-first 的一等对象。默认 session/terminal runtime 可以落在 project root；`Create Worktree Attachment` 是为了显式 materialize branch-isolated filesystem，而不是 basic runtime 的解锁条件。
...
- **Unmanaged Worktree surface**：live git worktree 若未绑定 Cell，应被明确提示并支持 `Create Cell`、可用时的 `Bind/Reattach <Cell>` 建议，以及 `Ignore For Now`。当 suggestion 指向 project-root Cell 时，verb 应是 `Bind`；当 suggestion 指向 detached Cell 时，verb 应是 `Reattach`；当 worktree 处于 detached HEAD 时，不应展示误导性的 `Create Cell` CTA，而应显式说明需要先附着到 branch。ignore 是 user-local 的噪声控制，不是 repo policy。
...
- **Workspace rail craft**：tracked/detached/unmanaged 区应保持同一套紧凑、面性、低噪声的 shell 语言；避免 card-within-card、重复 boxed chrome 或者为了“解释状态”堆出第二套 dashboard。project-root Cell 不应因为没 worktree 就掉进一个次级 rail。
...
- **Create vs Bind**：`Create Cell` 内部需要区分四种语义：创建 project-root Cell、Agency 新建 branch/worktree、绑定已有 worktree、绑定已有 branch。只有新建 branch/worktree 受 branch strategy / naming 约束；其余流程必须保留用户已有 branch identity，不把绑定流程伪装成新建流程。`Bind Existing Branch` 若找不到 live worktree，应只创建 project-root Cell；`Bind Branch` 到现有 project-root Cell 只更新 metadata；只有显式 `Create Worktree Attachment` 才能 materialize 新 worktree。
...
- `typeColors`: 旧状态色仍可作为兼容信息存在，但默认 core grouping 不再依赖 `draft/active/archived`。
