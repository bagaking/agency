...
## Manual Verification
1. 移除或 detach 一个非 `archived` 的 Cell worktree，确认 Agent Cells 将其移到 `Needs Cleanup` 区，保留 repo-owned sessions/evidence 摘要，且不再按普通 session tree 渲染。
2. 从该 cleanup card 执行 `Archive Cell` 并完成确认，确认该 Cell 离开 `Needs Cleanup`，进入 `Archived Offline` 区；此时主动作变为 `Review Details`（而不是继续显示 `Archive Cell`），repo-owned sessions/evidence 仍可从 details 访问。
