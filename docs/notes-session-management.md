...
- **Lifecycle rail visual language**：lifecycle card 应优先使用面性 shell 而不是高存在感 outline。推荐做法是低对比 filled background + 轻内部分隔 + 克制的状态 chips；selected 态用整体 surface 提亮和阴影质量表达，不要再靠粗边框或彩色外描边把卡片圈出来。
