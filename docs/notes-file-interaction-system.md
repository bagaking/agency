...
18. Bounded web research now stays inside existing Agency seams:
   Explorer owns URL intake and launch, Workbench owns the bounded web research tab, `View` is hosted as a true native browser surface instead of a renderer iframe, the tab keeps a compact two-tier toolbar so the remote page remains the dominant surface, reader previews save through the workbench file-writing path, saved Markdown files carry fixed `agency_source_*` frontmatter so Workbench can reopen them in markdown + preview mode, memo citations reuse HIL memo artifacts, in-view navigation stays constrained to the same public-URL policy as Explorer intake, and saved files hand back into standard Explorer/workbench open/reveal flows instead of creating a browser-local intake path while keeping cookies/session management and general browser tabs out of scope.
