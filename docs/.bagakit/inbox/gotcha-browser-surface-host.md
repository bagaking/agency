...
sources:
  ...
  - docs/notes-electron-browser-surface.md
created: 2026-04-02
---
...
- The April 5 packaged failure where `Go` appeared dead was this exact class of bug: the host rect reported `width > 0` and `height = 0`, so the failure happened before `loadURL`, not inside navigation.
...
- Keep the long-form canonical write-up in `docs/notes-electron-browser-surface.md`; this inbox note should stay short and source-linked.
