...
#### Scenario: Create worktree attachment for an existing project-root Cell
- **WHEN** a user explicitly requests worktree materialization for a project-root Cell
...
- **WHEN** a user starts a session on a project-root Cell that has no live worktree attachment yet
...
The editor SHALL distinguish tracked Cells, detached Cells, and unmanaged worktrees in Agent Cells.
...
#### Scenario: Project-root Cells share the tracked Cell rail
- **WHEN** a tracked Cell is running on the project root without a live worktree attachment
...
#### Scenario: Deterministic unmanaged match prefers bind for project-root Cells
- **WHEN** an unmanaged worktree has a deterministic match to an existing project-root Cell
...
#### Scenario: Tracked Cell does not revive stale branch metadata on detached HEAD
- **WHEN** a live tracked Cell reappears with an attached worktree that Git reports as detached HEAD
- **THEN** the Cell remains attached
- **AND** the UI surfaces `Detached HEAD` metadata instead of reviving stale branch text from stored Cell metadata
...
Workbench layout SHALL own the browser-lane geometry through one authoritative browser viewport host plus an explicit renderer-view seam rather than letting a tab-local content fragment, shell relay host, or raw renderer-bounds fallback act as the authoritative native-host placement source.
...
- **THEN** the browser surface reuses a Workbench-owned browser viewport host or split slot instead of measuring an arbitrary nested tab-local host node or shell proxy host
- **AND** the native browser content stays aligned with the same pane economy as the rest of the shell

#### Scenario: Missing renderer-view seam fails closed
- **WHEN** the main process cannot resolve renderer-view native bounds for the browser lane
- **THEN** the browser surface hides or suspends instead of trusting raw renderer coordinates
- **AND** Agency does not preserve stale browser placement
...
The editor SHALL allow a project-root tracked Cell to exist with no live worktree attachment and without treating that state as detached or missing.
...
#### Scenario: Select a project-root Cell
- **WHEN** a user selects a tracked Cell that is running on the project root with no live worktree attachment yet
- **THEN** the editor keeps terminal/session runtime available on the project root
- **AND** the editor still offers explicit branch-binding and worktree-attachment actions rather than auto-materializing a branch-isolated workspace
...
The Agent Cells sidebar SHALL project missing/detached non-archived Cells into a dedicated `Detached Cells` section rather than rendering them as ordinary session-tree cards.
The detached section SHALL use attachment-management copy, keep `View Details` as the primary CTA, and may expose archive as a direct secondary action.
Cells whose lifecycle state is already `archived` SHALL leave the detached section and active development buckets and become reachable through the explicit `Legacy Archived` compatibility surface.
Detached and archived compatibility surfaces SHALL stay visually compact, avoid nested card-within-card scaffolding, and rely on a single-shell action grammar rather than duplicated bordered wells.
...
#### Scenario: Detached Cells section in Agent Cells
...
- **THEN** those Cells appear in a dedicated `Detached Cells` section with preserved-evidence copy and session summary
- **AND** each detached Cell keeps `View Details` as the primary CTA
- **AND** the card may expose a direct archive action without reintroducing lifecycle cleanup rails
- **AND** the copy distinguishes `missing` attachment from intentionally `detached` attachment
...
- **THEN** the sidebar exposes an explicit `Legacy Archived` affordance
...
- **THEN** that Cell appears only through the archived-history surface instead of `Detached Cells`
