...
- [x] 1.2 Update session-management and product docs to describe the shell right-side rail as the queue surface, Session Map `Ops` as evidence-only, and Agent Cells as inline-only.
...
- [x] 3.3 Add destination-aware `Next` tooltip coverage and update manual verification wording, including the evidence-path destination branch and canonical-state wording.
