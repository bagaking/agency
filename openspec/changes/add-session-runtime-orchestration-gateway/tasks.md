...
- [x] 2.2 Promote programmatic session input delivery into one canonical host primitive (`dispatchSessionInput`-style contract) and stop duplicating ad hoc launch logic.
- [x] 2.3 Add reusable wait-condition helpers for output pattern, quiet window, and ready-state checks.
- [x] 2.4 Add a host-owned child-session creation/launch helper so higher-level orchestration does not rebuild topology + launch steps in renderer code.
...
- [x] 3.2 Implement the `codex` smart fork driver:
  ...
  - child ready wait.
- [x] 3.3 Wire Agent Cells `Fork` action to the new gateway-backed path and preserve clear fallback/error behavior.
